...
이 안내서를 사용하여 RustDesk Server Pro가 OAuth2를 통해 Microsoft 365 Exchange Online를 통해 이메일을 보내도록 구성하십시오.

이 설정은 초대 이메일, 로그인 인증 이메일 및 연결 알람 알림에 적합합니다.

일반 SMTP 설정에 대해서는 [SMTP](../)를 참조하십시오.

## RustDesk Pro에 어떤 값이 들어가나요?
...
- 기존의 Microsoft 365 사서함 또는 메일 발송용으로 생성할 예정인 사서함, 예를 들어 `no-reply@contoso.com`
- Microsoft Entra에서 관리자 동의를 부여하고 Exchange Online 서비스 원칙을 관리할 수 있는 Microsoft 365 관리자 계정

이 구성은 세 부분으로 나뉩니다:

- Azure에서 앱 등록, 클라이언트 비밀번호, API 권한 및 관리자 동의 구성
- PowerShell에서 Exchange Online 서비스 원칙, 사서함 및 권한 구성
...
1. 검색하여 **앱 등록**을 선택합니다.
1. 왼쪽 메뉴에서 [**앱 등록**](https://portal.azure.com/#view/Microsoft_AAD_IAM/ActiveDirectoryMenuBlade/~/RegisteredApps)을 선택한 후, **새로 등록**을 클릭합니다.
...
1. **인증서 및 비밀번호**를 열고 새 클라이언트 비밀번호를 생성합니다.
...
1. **API 권한**을 열고 Microsoft 365 Exchange Online SMTP 애플리케이션 권한을 추가합니다.
1. **권한 추가**를 선택합니다.
1. **내 조직에서 사용하는 API**를 선택하고 **Office 365 Exchange Online**을 검색합니다.
1. **애플리케이션 권한**을 선택합니다.
1. **SMTP.SendAsApp**를 선택하고 변경사항을 저장합니다.
...
1. Exchange Online 모듈을 설치하고 테넌트 관리자 계정으로 연결합니다.
...
1. Microsoft Entra **기업용 애플리케이션**에서 앱을 찾아 `Object ID`를 기록해 둡니다. Exchange Online 서비스 주체를 생성할 때 필요합니다.

{{% notice note %}}
여기서 `OBJECT_ID`는 **기업용 애플리케이션**의 앱 객체 ID여야 하며, **앱 등록** 개요 페이지에 표시된 객체 ID가 아닙니다.
{{% /notice %}}

![](/docs/en/self-host/rustdesk-server-pro/smtp/microsoft-365/images/15-Powershell-Get-ObjectId.png)

1. 이 명령어를 실행하여 앱 등록용 Exchange Online 서비스 주체를 생성합니다. Microsoft는 이 단계를 Exchange Online에서 Microsoft Entra 애플리케이션의 서비스 주체 등록이라고 설명합니다.

```powershell
New-ServicePrincipal -AppId <APPLICATION_ID> -ObjectId <OBJECT_ID>
```

이 명령어가 성공했음에도 불구하고 Exchange 연결이 실패한다면, 관리자 계정에 Exchange Online 서비스 주체를 관리할 권한이 있는지 확인하세요.
![](/docs/en/self-host/rustdesk-server-pro/smtp/microsoft-365/images/16-Powershell-New-ServicePrincipal.png)

1. Exchange가 서비스 주체를 생성했는지 확인하고, 다음 단계를 위해 `Identity` 값을 기록해 두세요.

```powershell
Get-ServicePrincipal | Format-Table DisplayName,AppId,ObjectId,Identity
```

여기서 반환된 `Identity` 값을 다음 두 권한 명령어의 `<SERVICE_PRINCIPAL_ID>`로 사용하세요.

1. 보낼 사서함이 아직 존재하지 않는다면, 먼저 공유 사서함을 생성할 수 있습니다. 예를 들면:

```powershell
New-Mailbox -Shared -Name "No Reply" -Alias no-reply -DisplayName "No Reply" -PrimarySmtpAddress no-reply@contoso.com
```

이미 메일을 보내는 사서함이 있다면 이 단계를 건너뛸 수 있습니다.
![](/docs/en/self-host/rustdesk-server-pro/smtp/microsoft-365/images/17-Powershell-New-Mailbox.png)

1. 테넌트와 보낼 사서함에 대해 `Authenticated SMTP`가 활성화되었는지 확인하세요.
...
활성화되지 않았다면 테스트 메일은 다음과 같은 오류로 실패할 수 있습니다:
...
사서함 수준 설정을 위해 필요하다면 다음을 실행하세요:

```powershell
Set-CASMailbox -Identity "no-reply@contoso.com" -SmtpClientAuthenticationDisabled $false
```

테넌트 수준 설정에서 `True`가 반환된다면 조직 정책에 따라 다음을 결정하세요:

```powershell
Set-TransportConfig -SmtpClientAuthenticationDisabled $false
```

위의 두 설정이 모두 올바르게 보이지만 동일한 `535 5.7.139` 오류가 계속된다면, 테넌트가 Microsoft Entra `Security defaults`를 사용하는지도 확인하세요. Microsoft Learn에 따르면, `Security defaults`가 활성화되면 Exchange Online에서 SMTP AUTH가 비활성화됩니다.

명령어 상세 내용은 Microsoft Learn: [Exchange Online에서 인증된 클라이언트 SMTP 제출(SMTP AUTH) 활성화 또는 비활성화](https://learn.microsoft.com/en-us/Exchange/clients-and-mobile-in-exchange-online/authenticated-client-smtp-submission)를 참조하세요.

1. RustDesk가 메일을 보내는 데 사용할 사서함에 Exchange 서비스 주체 `FullAccess`를 부여하세요.

```powershell
Add-MailboxPermission -Identity "no-reply@contoso.com" -User <SERVICE_PRINCIPAL_ID> -AccessRights FullAccess
```

RustDesk에 입력할 사서함을 사용하세요 `Mail Account`.
...
- 클라이언트 시크릿 `Value`
- 이미 확인된 Exchange Online 서비스 주체로, `FullAccess`와 `SendAs`가 `Mail Account`에 사용된 사서함에 이미 설정되어 있습니다

RustDesk는 Exchange 서비스 주체 `Identity`를 요구하지 않지만, 메일을 테스트하기 전에 위의 권한 단계가 이미 완료되어 있어야 합니다.

1. RustDesk [웹 콘솔](../../console/)에서 **설정** -> **SMTP**로 이동하세요.
...
1. **확인**을 클릭하여 설정을 저장하고 테스트 메일을 보내세요.
![](/docs/en/self-host/rustdesk-server-pro/smtp/microsoft-365/images/20-RustDesk-SMTP-OAuth2-1.png)
![](/docs/en/self-host/rustdesk-server-pro/smtp/microsoft-365/images/21-RustDesk-SMTP-OAuth2-2.png)

테스트 메일이 여전히 실패한다면, PowerShell 섹션으로 돌아가 Exchange Online 서비스 주체, `Authenticated SMTP` 및 `Mail Account`에 사용된 사서함의 권한을 다시 확인하세요.

## 참고문헌

- Microsoft Learn: [OAuth를 사용하여 IMAP, POP 또는 SMTP 연결 인증](https://learn.microsoft.com/en-us/exchange/client-developer/legacy-protocols/how-to-authenticate-an-imap-pop-smtp-application-by-using-oauth). Exchange Online 앱 권한 및 서비스 주체 단계에 사용됨.
- Microsoft Learn: [Exchange Online에서 인증된 클라이언트 SMTP 제출(SMTP AUTH) 활성화 또는 비활성화](https://learn.microsoft.com/en-us/Exchange/clients-and-mobile-in-exchange-online/authenticated-client-smtp-submission). `Authenticated SMTP` 확인 및 활성화에 사용됨.
- Microsoft Learn: [Exchange 관리 센터에서 공유 사서함 만들기](https://learn.microsoft.com/en-us/exchange/collaboration/shared-mailboxes/create-shared-mailboxes). 공유 사서함 생성에 사용됨.
