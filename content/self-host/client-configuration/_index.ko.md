---
title: 클라이언트 구성
description: "셀프 호스팅 서버용 RustDesk 클라이언트를 구성하세요. 브랜드 앱에 로고를 포함한 사용자 지정 클라이언트(Custom Client) 생성기(Pro), 수동 구성, 설정 가져오기/내보내기 및 엔터프라이즈 배포 전략과 같은 기능을 제공합니다."
...
- `Relay Server`: 종종 선택 사항입니다. RustDesk가 이를 추론할 수 있기 때문이며, 명시적으로 설정하려는 경우를 제외하고는 그렇습니다.

## 1. 사용자 지정 클라이언트 생성기(Pro 전용, 기본 플랜 또는 사용자 지정 플랜)

고유한 이름, 로고, 아이콘, 구성 설정을 갖추고 서명 및 기타 기능을 추가할 수 있습니다.

현재 Windows X64, Mac Arm64 / X64, [Linux](https://twitter.com/rustdesk/status/1788905463678951787), Android Arm 64가 지원됩니다.

[영상](https://twitter.com/rustdesk/status/1769171628426944539)
...
메인 RustDesk 클라이언트 홈에서 ID 옆에 있는 메뉴 버튼 [ &#8942; ]을 클릭한 다음 네트워크를 클릭하세요. 이제 상승된 권한을 사용해 설정을 잠금 해제하고 `ID`, `Relay`, `API` 및 `Key`를 설정할 수 있습니다. 여기서 주의해야 할 점은 이 `Key`가 연결 암호화에 사용되는 공개 키이며, Pro 버전 구매 시 제공되는 라이선스 키와는 구분된다는 것입니다.

![](/docs/en/self-host/client-configuration/images/network-config.png)

**ID 서버** 입력 상자에 `hbbs` 호스트 또는 IP 주소를 입력하세요(로컬 측 + 원격 측). 다른 두 개의 주소는 비워두어도 되며, RustDesk가 자동으로 추론합니다(특별히 설정하지 않은 경우). 릴레이 서버는 `hbbr`(포트 21117)를 가리킵니다.
...
자신이 호스팅하는 서버와 암호화된 연결을 설정하려면 해당 서버의 공개 키를 입력해야 합니다. 이 키는 보통 `hbbs`를 처음 실행할 때 생성되며, 작업 디렉터리/데이터 폴더의 `id_ed25519.pub` 파일에서 확인할 수 있습니다.

`Pro` 사용자는 추가로 [웹 콘솔](https://rustdesk.com/docs/en/self-host/rustdesk-server-pro/console/)에서 `Key`를 가져올 수 있습니다.

![](/docs/en/self-host/rustdesk-server-pro/console/images/console-home.png?v2)

### `API Server` 설정하기

이는 `Pro` 사용자만을 위한 것입니다. 웹 콘솔에 로그인할 수 있지만 RustDesk 클라이언트에 로그인하지 못한다면, 아마도 `API Server`를 올바르게 설정하지 않았을 가능성이 있습니다.

API 서버가 기본 `21114` 포트에서 실행되지 않는다면(오픈소스 버전에서 오는 경우 방화벽에 이 포트를 추가하지 않을 수도 있음), 명시적으로 `API Server`를 지정해 주세요.
예: API 서버가 기본 HTTPS 포트에서 실행된다면, `API Server`를 `https://hbbs.example.com`와 함께 지정해 주세요.

여전히 `API Server`의 값을 확인할 수 없다면, 웹 콘솔의 환영 페이지로 이동하세요. 위 그림에 표시된 `API Server`가 바로 입력 상자입니다(라벨이 `API:`인 입력 상자).

## 3. 가져오기 또는 내보내기를 사용한 설정

1. 위의 단계를 따라 [위](https://rustdesk.com/docs/en/self-host/client-configuration/#manual-config)에서 설명한 대로 장치에 RustDesk 클라이언트를 구성하십시오.
2. 위의 기기에서 설정으로 이동한 다음 네트워크로 이동하고 잠금을 해제하십시오.
3. `Export Server Config`를 클릭하십시오.
4. 복사된 문자열을 메모장이나 유사한 프로그램에 붙여넣으십시오.
...
자동으로 설정하는 가장 쉬운 방법은 [여기](https://rustdesk.com/docs/en/self-host/client-deployment/)에서 찾을 수 있는 배포 스크립트를 사용하는 것입니다.
...
웹 콘솔에서 구성 문자열을 얻을 수 있습니다(위 그림에서 확인할 수 있음) 또는 RustDesk 클라이언트의 "설정 → 네트워크"에서 얻을 수 있습니다([여기](https://github.com/rustdesk/rustdesk/discussions/7118)는 이에 대한 토론입니다).
