...
title: 시놀로지
weight: 22
description: "Synology NAS에서 RustDesk 서버 OSS를 셀프호스팅하세요. DSM 6 또는 DSM 7.2 컨테이너 관리자에 맞는 가이드를 선택하고, 지원되는 Synology 모델에서 Docker를 사용해 배포하세요."
...
1. NAS가 DSM 6 또는 DSM 7.2+를 실행하는지 확인하십시오.
2. 해당 DSM 버전에 맞는 올바른 Docker 또는 Container Manager 패키지를 설치하십시오.
3. 해당 패키지에 맞는 RustDesk 배포 가이드를 따르십시오.
4. 필요한 RustDesk 포트를 열고 컨테이너 데이터의 지속적 저장소를 확인하십시오.

Portainer와 함께 Synology를 사용하는 경우, [이 튜토리얼](https://mariushosting.com/how-to-install-rustdesk-on-your-synology-nas/)을 참조하십시오.
